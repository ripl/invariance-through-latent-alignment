...
with Sweep(Args, Adapt) as sweep:
    ...
    # Critical
    Adapt.encoder_from_scratch = True
    Adapt.use_discr_score = False


    with sweep.zip:
        Args.env_name = [f'dmc:{env_name}-v1' for env_name in envs] + [f'dmc:{env_name}-v1' for env_name in envs]
        # Args.eval_env_name = [f'dmc:{env_name}-v1' for env_name in envs]
        Args.eval_env_name = [f'dmc:{env_name}-v1' for env_name in envs] + [f'distracting_control:{env_name}-intensity-v1' for env_name in envs]
        Adapt.distraction_types = [[] for _ in envs] + [['background'] for _ in envs]
        Adapt.distraction_intensity = [0.0 for _ in envs] + [1.0 for _ in envs]
...
def tail(Args, Adapt):
    ...
    Args.job_name = f"latent_vis_encscr/{Adapt.agent_stem}/{buffer_prefix}/ganlr{Adapt.gan_lr}/gandiscrlr{Adapt.gan_disc_lr}/{Args.seed}"


sweep.save("latent_vis_encscr.jsonl")
