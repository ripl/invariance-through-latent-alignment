...
with Sweep(Args, Adapt) as sweep:
    ...
    Adapt.fwdm_pretrain_lr = 5e-4
    Adapt.invm_kickin_step = 0
    Adapt.num_invm_steps = 2_000
    ...
    with sweep.zip:
        Adapt.fwdm_lr = [0, 1e-3, 1e-4, 1e-5]
        Adapt.invm_lr = [0, 1e-3, 1e-4, 1e-5]
...
def tail(Args, Adapt):
    ...
    Args.job_name = f"adapt-dynonly-noaug/sample-bugfix/adapt/{Adapt.agent_stem}/{buffer_prefix}/fwdmlr{Adapt.fwdm_lr}/invmlr{Adapt.invm_lr}/kickin{Adapt.invm_kickin_step}/{Args.seed}"
