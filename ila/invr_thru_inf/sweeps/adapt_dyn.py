...
with Sweep(Args, Adapt) as sweep:
    ...
    with sweep.zip:
        Adapt.invm_pretrain_lr = [1e-4, 5e-4, 1e-3]
        Adapt.fwdm_pretrain_lr = [1e-4, 5e-4, 1e-3]

    with sweep.zip:
        Adapt.fwdm_lr = [1e-4, 5e-4]
        Adapt.invm_lr = [1e-4, 5e-4]
    ...
    with sweep.product:
        ...
        Adapt.adapt_dyn_model = [False, True]
...
def tail(Args, Adapt):
    ...
    Args.job_name = f"adapt-dyn-noaug-0426-1724/sample-bugfix/adapt/{Adapt.agent_stem}/{buffer_prefix}/fwdmlr{Adapt.fwdm_lr}/invmlr{Adapt.invm_lr}/kickin{Adapt.invm_kickin_step}/{Args.seed}"


sweep.save("adapt_partial_dynonly_noaug.jsonl")
