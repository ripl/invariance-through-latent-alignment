...
def tail(Args, Adapt):
    ...
    buffer_prefix = str(get_buffer_prefix(Args.env_name, Args.action_repeat, Args.seed, snapshot_prefix=Adapt.snapshot_prefix,
                                          distraction_types=Adapt.distraction_types, distraction_intensity=Adapt.distraction_intensity,
                                          augment=Adapt.augment, eval_env=Args.eval_env_name))[:-4]
    Args.job_name = f"latent_vis2/{Adapt.agent_stem}/{buffer_prefix}/ganlr{Adapt.gan_lr}/gandiscrlr{Adapt.gan_disc_lr}/{Args.seed}"


sweep.save("latent_vis_video.jsonl")
